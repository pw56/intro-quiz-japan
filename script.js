function generateMusicPrompt(startDate, endDate, numSongs, isSecondTime = false) {
  let prompt =
`${startDate}から${endDate}の間に以下のサイトで上位100位以内にランクインした楽曲(J-POP, K-POP, 洋楽など)をバランスよく調査し、ランダムな順で重複がないように${numSongs}曲、日本版の曲名とアーティストを列挙してから、列挙したものを指定したYAML形式でフォーマットしてください。推論の過程をプロンプトに出力してください。サンプルではなく${numSongs}曲全ての曲を書式化するまで繰り返してください。要件を満たした出力結果になるまで検索・推論を繰り返してください。実行時間はいくらかかっても構いません。
